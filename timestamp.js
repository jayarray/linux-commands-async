class Timestamp {
  static Now(executor) {
    return new Promise((resolve, reject) => {
      ...
      let meridiemTimeInfoCmd = `date +'%I %M %S %N %P'`; // NOTE: %P (i.e. suffix) may not be known in some places.

      // EXECUTE COMMANDS
      let cmdList = [dayInfoCmd, weekInfoCmd, monthInfoCmd, yearInfoCmd, meridiemTimeInfoCmd];
      let cmdListStr = cmdList.join('\n');
      executor.Execute(cmdListStr, []).then(output => {
        if (output.stderr) {
